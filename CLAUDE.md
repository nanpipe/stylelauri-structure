...
Recommended store status slugs per role (the store adds the `wc-` prefix): `abono`→saldo-pendiente, `produccion`→abono-produccion, `preventa`→preventa, `listo`→preparacion, `boletas`→boleta-confirmada. The `boletas` string is only the internal ROLE key (`slo_status_boletas` option); the store status it maps to can be any slug — there is no hardcoded status slug.
